...
def alexnet_model(input_dim, output_dim, 
            num_filters1, filter_size1, conv_stride1, 
            pool_size1, pool_stride1,
            num_filters2, filter_size2, conv_stride2, 
            pool_size2, pool_stride2,
            num_filters3, filter_size3, conv_stride3, 
            num_filters4, filter_size4, conv_stride4,
            num_filters5, filter_size5, conv_stride5,
            pool_size3, pool_stride3, 
    ...
    conv5 = conv_1d_block(conv4, num_filters5, filter_size5, conv_stride5)
    # 3 POOLS 
    pool3 = layers.MaxPooling1D(pool_size3, pool_stride3, padding='same')(conv5)
...
def alexnet_bo(input_data, output_data,
               ...
               dense_units1, dense_units2,
               optimizer, learning_rate, batch_size, epochs=100):
...
params_alexnet = {
    ...
    'num_filters3': (1, 128),  # Number of filters in the third convolutional layer
    'num_filters4': (1, 128),  # Number of filters in the fourth convolutional layer
    'num_filters5': (1, 128),  # Number of filters in the fifth convolutional layer
...
print("Model execution time train model is: ", execution_time1)
...
scores = alexnet_best_model.evaluate(x_val_aug, y_val_aug, verbose=0)
...
plt.plot(fpr[0], tpr[0], color = 'darkorange', lw = 1, 
         label = 'Gayo (AUC = %0.2f)' % roc_auc[0])
plt.plot(fpr[1], tpr[1], color = 'navy', lw = 1, 
         label = 'Kintamani (AUC = %0.2f)' % roc_auc[1])
plt.plot(fpr[2], tpr[2], color = 'deeppink', lw = 1, 
         label = 'Temanggung (AUC = %0.2f)' % roc_auc[2])
plt.plot(fpr[3], tpr[3], color = 'green', lw = 1, 
         label = 'Toraja (AUC = %0.2f)' % roc_auc[3])
