...
def lenet_model(input_dim, output_dim,
    ...
    pool1 = layers.AveragePooling1D(pool_size=pool_size1, strides=pool_stride1, padding='same')(conv1)
    # BLOCK 1: CONV -> RELU -> POOL 
    conv2 = conv_1d_block(pool1, num_filters2, filter_size2, conv_stride2)
    pool2 = layers.AveragePooling1D(pool_size=pool_size2, strides=pool_stride2, padding='same')(conv2)
...
lenet_best_model.save(f'{output_path}{model_name}_best_model.keras')
lenet_best_model.save_weights(f'{output_path}{model_name}_best_model.model.weights.h5')
...
scores = lenet_best_model.evaluate(x_val_aug, y_val_aug, verbose=0)
